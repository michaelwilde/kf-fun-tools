...
// Set focus immediately when page loads and auto-trigger 'about' after 1 second
window.addEventListener("load", () => {
  ...
  setTimeout(() => {
    ...
    }, 200); // Type each character with 200ms delay
  }, 1000); // 1 second
});
      ...
      if (arg && exploreOptions[arg]) {
        ...
        setTimeout(() => {
          ...
          if (terminal.style.color === "#fff") {
            applyRainbowText(
              tabDiv,
              "Opening a new tab, don’t worry terminal is still here, come back and hang out!",
            );
          } else {
            tabDiv.textContent =
              "Opening a new tab, don’t worry terminal is still here, come back and hang out!";
          ...
          case "joshua":
            ...
            setTimeout(() => {
              ...
              if (terminal.style.color === "#fff") {
                applyRainbowText(
                  launchDiv,
                  "Simulating Global Thermonuclear War...",
                );
              } else {
                launchDiv.textContent =
                  "Simulating Global Thermonuclear War...";
              }
...
function startBrickoutGame() {
  output.innerHTML =
    "<div>Starting Brickout... Use ← and → keys to move the paddle! Esc or Ctrl-C to exit.</div>";
  if (terminal.style.color === "#fff") {
  ...
  const brickOffsetLeft =
    (WIDTH - (brickColumnCount * (brickWidth + brickPadding) - brickPadding)) /
    2;
...
function startTetrisGame() {
  output.innerHTML =
    "<div>Starting Tetris... Use ← and → to move, ↑ to rotate, ↓ to drop! Esc or Ctrl-C to exit.</div>";
  if (terminal.style.color === "#fff") {
  ...
  function drawShape() {
    for (let r = 0; r < currentShape.length; r++) {
      for (let c = 0; c < currentShape[0].length; c++) {
        // Use currentShape[0].length for width
        if (currentShape[r][c]) {
          const drawY = currentY + r;
          if (drawY >= -1) {
            // Draw if at least partially visible
            ctx.fillStyle =
              colors[
                shapes.indexOf(
                  shapes.find((s) => s.toString() === currentShape.toString()),
                )
              ];
            ctx.fillRect(
              offsetX + (currentX + c) * gridSize,
              (drawY + 4) * gridSize,
              gridSize - 1,
              gridSize - 1,
            );
  ...
  function draw() {
    ...
    if (moveCooldown <= 0 && !justMerged) {
      // Don't move if just merged
...
function startMDRGame() {
  output.innerHTML =
    "<div>Starting Macrodata Refinement... Click a number, then a bin to sort! Esc or Ctrl-C to exit.</div>";
  if (terminal.style.color === "#fff") {
  ...
  const bins = Array(binCount)
    ...
    .map(() => ({
      ...
      fc: 0,
      dr: 0,
      ma: 0, // Woe, Frolic, Dread, Malice progress
    }));
    ...
    if (selectedNumber) {
      for (let i = 0; i < binCount; i++) {
        ...
        if (
          ...
          const bin = bins[i];
          const temper = ["wo", "fc", "dr", "ma"][
            Math.floor(Math.random() * 4)
          ];
          bin[temper] += Math.floor(Math.random() * 20) + 10; // Random progress boost
          numbers[selectedNumber.r][selectedNumber.c] = Math.floor(
            Math.random() * 10,
          ); // Replace sorted number
          selectedNumber = null;
          if (getTotalProgress(bin) >= 100) {
            output.innerHTML +=
              "<div>File refinement complete! Type anything to continue.</div>";
            if (terminal.style.color === "#fff") {
  ...
  function draw() {
    ...
    const totalProgress =
      (bins.reduce((sum, bin) => sum + getTotalProgress(bin), 0) /
        (binCount * 100)) *
      100;
    ...
    for (let r = 0; r < gridRows; r++) {
      for (let c = 0; c < gridCols; c++) {
        ...
        ctx.fillStyle =
          selectedNumber && selectedNumber.r === r && selectedNumber.c === c
            ? "#ff0"
            : "#fff";
        ctx.font = "18px Roboto Mono";
        ctx.textAlign = "center";
    ...
    for (let i = 0; i < binCount; i++) {
      ...
      ctx.fillRect(
        x + 5,
        y + 25,
        (Math.min(bin.wo, 100) / 100) * barWidth,
        barHeight,
      );
      ...
      ctx.fillRect(
        x + 5,
        y + 40,
        (Math.min(bin.fc, 100) / 100) * barWidth,
        barHeight,
      );
      ...
      ctx.fillRect(
        x + 5,
        y + 55,
        (Math.min(bin.dr, 100) / 100) * barWidth,
        barHeight,
      );
      ...
      ctx.fillRect(
        x + 5,
        y + 70,
        (Math.min(bin.ma, 100) / 100) * barWidth,
        barHeight,
      );
